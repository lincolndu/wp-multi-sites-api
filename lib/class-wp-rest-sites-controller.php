...
class WP_REST_Sites_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'args'   => array(
					'id' => array(
						'description' => __( 'Unique identifier for the object.','multisiteapi' ),
						'type'        => 'integer',
					),
				),
				array(
				...
				array(
					...
					'args'                => array(
						'force'             => array(
							'type'        => 'boolean',
							'default'     => true,
							'description' => __( 'Whether to bypass trash and force deletion.','multisiteapi' ),
						),
						'uninitialize_site' => array(
							'type'        => 'boolean',
							'default'     => true,
							'description' => __( 'Whether to  uninitialize site, drop tables and delete uploads.','multisiteapi' ),
						),
					),
				),
				'schema' => array( $this, 'get_public_item_schema' ),
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! is_multisite() ) {
			return new WP_Error( 'rest_multisite_not_installed', __( 'Multisite is not installed','multisiteapi' ), array( 'status' => 400 ) );
		}

		if ( ! current_user_can( $this->user_can ) ) {
			return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit sites.','multisiteapi' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	protected function get_site( $id ) {
		...
		$error = new WP_Error( 'rest_site_invalid_id', __( 'Invalid site ID.','multisiteapi' ), array( 'status' => 404 ) );
		if ( (int) $id <= 0 ) {
			return $error;
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! empty( $request['context'] ) && 'edit' === $request['context'] && ! current_user_can( $this->user_can ) ) {
			return new WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit sites.','multisiteapi' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! empty( $request['id'] ) ) {
			return new WP_Error( 'rest_site_exists', __( 'Cannot create existing site.','multisiteapi' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $site_id ) {
			return new WP_Error( 'rest_site_failed_create', __( 'Creating site failed.','multisiteapi' ), array( 'status' => 500 ) );
		}
		...
		$context = current_user_can( $this->user_can ) ? 'edit' : 'view';
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! $this->check_edit_permission( $site ) ) {
			return new WP_Error( 'rest_cannot_edit', __( 'Sorry, you are not allowed to edit this site.','multisiteapi' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( ! empty( $prepared_args['network'] ) ) {
			if ( ! get_network( $prepared_args['network'] ) ) {
				return new WP_Error( 'rest_network_id_invalid', __( 'Invalid network ID.','multisiteapi' ), array( 'status' => 400 ) );
			}
		}
		...
		if ( ! empty( $schema['properties']['meta'] ) && isset( $request['meta'] ) ) {
			if ( function_exists( 'is_site_meta_supported' ) && ! is_site_meta_supported() ) {
				...
				return new WP_Error(
					...
					sprintf( __( 'The %s table is not installed. Please run the network database upgrade.','multisiteapi' ), $GLOBALS['wpdb']->blogmeta ),
					array( 'status' => 500 )
				);
			}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'delete_sites' ) ) {
			return new WP_Error( 'rest_cannot_delete', __( 'Sorry, you are not allowed to delete this site.','multisiteapi' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'network'      => array(
					'description' => __( 'The site\'s network ID. Default is the current network ID.','multisiteapi' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit', 'embed' ),
				),
				'domain'       => array(
					'description' => __( ' Site domain,','multisiteapi' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit', 'embed' ),
					'default'     => '',
				),
				...
				'registered'   => array(
					'description' => __( 'When the site was registered, in SQL datetime format. Default is the current time.','multisiteapi' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit', 'embed' ),
				),
				'last_updated' => array(
					'description' => __( 'When the site was last updated, in SQL datetime format. Default isthe value of $registered.','multisiteapi' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit', 'embed' ),
				),
				'public'       => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( 'Whether the site is public. Default 1.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 1,
				),
				'archived'     => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( 'Whether the site is archived. Default 0.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 0,
				),
				'mature'       => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( 'Whether the site is mature. Default 0.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 0,
				),
				'spam'         => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( ' Whether the site is spam. Default 0.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 0,
				),
				'deleted'      => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( 'Whether the site is deleted. Default 0.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 0,
				),
				'lang_id'      => array(
					'context'     => array( 'view', 'edit', 'embed' ),
					'description' => __( 'The site\'s language ID. Currently unused. Default 0.','multisiteapi' ),
					'type'        => 'integer',
					'default'     => 0,
				),
				'blogname'     => array(
					'description' => __( 'Site\'s name, stored in blogname option','multisiteapi' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'siteurl'      => array(
					'description' => __( 'Site\'s site url, stored in site_url option','multisiteapi' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'home'         => array(
					'description' => __( 'Site\'s home url, stored in hom option','multisiteapi' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'post_count'   => array(
					'description' => __( 'Number of posts on this site','multisiteapi' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'default'     => 0,
				),
	...
	public function get_collection_params() {
		...
		$query_params['domain'] = array(
			'description' => __( 'Limit result set to sites assigned to specific domain. ','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'string',
			),
		...
		$query_params['domain_exclude'] = array(
			'description' => __( 'Ensure result set excludes sites assigned to specific domain. ','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'string',
			),
		...
		$query_params['path'] = array(
			'description' => __( 'Limit result set to sites assigned to specific path. ','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'string',
			),
		...
		$query_params['path_exclude'] = array(
			'description' => __( 'Ensure result set excludes sites assigned to specific path. ','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'string',
			),
		...
		$query_params['exclude'] = array(
			'description' => __( 'Ensure result set excludes specific IDs.','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$query_params['include'] = array(
			'description' => __( 'Limit result set to specific IDs.','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$query_params['offset'] = array(
			'description' => __( 'Offset the result set by a specific number of items.','multisiteapi' ),
			'type'        => 'integer',
		);

		$query_params['order'] = array(
			'description' => __( 'Order sort attribute ascending or descending.','multisiteapi' ),
			'type'        => 'string',
			'default'     => 'desc',
			'enum'        => array(
				'asc',
		...
		$query_params['orderby'] = array(
			'description' => __( 'Sort collection by object attribute.','multisiteapi' ),
			'type'        => 'string',
			'default'     => 'id',
			'enum'        => array(
				'id',
		...
		$query_params['network'] = array(
			'default'     => array(),
			'description' => __( 'Limit result set to sites of specific network IDs.','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
		...
		$query_params['network_exclude'] = array(
			'default'     => array(),
			'description' => __( 'Ensure result set excludes specific network IDs.','multisiteapi' ),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
	...
	protected function check_read_permission( $site, $request ) {
		...
		return current_user_can( $this->user_can );
	}
